...
#' @export
getActivity<-function(gsc,dat,size.min=2,es.method='mean',restand=TRUE){
  ...
  if(class(dat)=='ExpressionSet'){
    ac<-getActivity4eset(gsc,dat,es.method=es.method,restand=restand)
  }

  else if(is.data.frame(dat)|is.matrix(dat)){

    ac<-getActivity4tbl(gsc,dat,es.method=es.method,restand=restand)
...
getActivity4tbl<-function(gsc,exp,es.method='mean',restand=TRUE){

  ac<-matrix(NA,nrow=ncol(exp),ncol=length(gsc),dimnames = list(colnames(exp),names(gsc)))

  if(restand) exp<-apply(exp,2,std)
...
getActivity4eset<-function(gsc,eset,es.method='mean',restand=TRUE){

  require(Biobase)

  ac<-getActivity4tbl(gsc,exprs(eset),es.method=es.method,restand=restand)
