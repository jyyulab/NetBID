...
if('pca' %in% choose_plot){
  if(pca_plot_type=='2D.interactive') {
    l <- htmltools::tagList()
     for (i in 1:length(intgroup)) {
       w1 <- unique(get_obs_label(pData(eset),intgroup[i]))
       if(length(w1)<=6){
                l[[i]] <- draw.pca.kmeans(use_mat,obs_label=get_obs_label(pData(eset),intgroup[i]),
                                 verbose=FALSE,main=intgroup[i],plot_type = pca_plot_type,use_color=use_color,pre_define=pre_define)
       }else{
         message(sprintf('No support for original clusters size larger than 6 (here %s has %s unique groups), please directly call draw.pca.kmeans() or draw.2D.interactive() for visualization! ',intgroup[i],length(w1)))
       }
     }
    plotly:::layout(l)
  }
}else{
    message('No selection for PCA plot!')
}
```
...
if('pca' %in% choose_plot){
  if(pca_plot_type!='2D.interactive') {
      ...
      for (i in 1:length(intgroup)) {
       tmp1 <- draw.pca.kmeans(use_mat,obs_label=get_obs_label(pData(eset),intgroup[i]),
                               verbose=FALSE,point_cex=pp,main=intgroup[i],plot_type = pca_plot_type)
     }
  }
}else{
    message('No selection for PCA plot!')
...
## Part IV: Correlation plot (`r correlation_strategy`)
