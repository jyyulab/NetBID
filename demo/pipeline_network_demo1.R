...
# direct modifications for the net_eset
#mat <- exprs(net_eset)
#phe <- pData(net_eset)
#gen <- fData(net_eset)
#ann <- annotation(net_eset)
# save to eset; or can user defined input by preparing the required info
#net_eset <- generate.eset(exp_mat=mat, phenotype_info=phe, feature_info=gen, annotation_info=ann)
# if no addition info, phe and gen can be null, will use rownames, colnames from mat
#net_eset <- generate.eset(exp_mat=mat, phenotype_info=NULL, feature_info=NULL, annotation_info='')

# ID conversion, or merge transcript level to expression level, use_feature_info can be other dataframe info; optional;
net_eset <- update.eset.feature(use_eset=net_eset,use_feature_info=fData(net_eset),from_feature='ID',to_feature='GENE_SYMBOL',merge_method='median') ### !!! need modify
# select phenotype columns or user add phenotype info; optional
net_eset <- update.eset.phenotype(use_eset=net_eset,use_phenotype_info=pData(net_eset),use_sample_col='geo_accession',use_col='GEO-auto')

## from local RNASeq data
## need to follow the tximport procedures + DESeq2, we only provide demo functions to load salmon output
# tximport(), DESeqDataSetFromTximport()
# need to prepare small size salmon output ?
#phe_info <- read.delim('/Volumes/project_space/IO_JY//yu3grp/EphA2CART/data/sample_info_GSE121226.txt',stringsAsFactors=FALSE)
#salmon_dir='/Volumes/project_space/IO_JY//yu3grp/EphA2CART/result/RNASeq_GSE121226'
#net_eset_RNASeq <- load.exp.RNASeq.demoSalmon(salmon_dir=salmon_dir,use_phenotype_info=phe_info,use_sample_col='Run',use_design_col='genotype',merge_level='transcript',return_type='eset')
...
plot.eset.QC(network.par$net.eset,outdir=network.par$out.dir.QC,intgroup=NULL,do.logtransform=FALSE,prefix='beforeQC_')

# save to RData
NetBID2.saveRData(network.par = network.par,step='exp-load')

######################################################### Step2: normalization for the exp dataset (exp-QC)
# load from RData
NetBID2.loadRData(network.par = network.par,step='exp-load')
...
# if intgroup==NULL, will auto get intgroup
# prefix: prefix for the pdf files
plot.eset.QC(network.par$net.eset,outdir=network.par$out.dir.QC,intgroup=NULL,do.logtransform=FALSE,prefix='afterQC_')

# save to RData
NetBID2.saveRData(network.par = network.par,step='exp-QC')

######################################################### Step3: check sample cluster info, optional (exp-cluster)
# load from RData
NetBID2.loadRData(network.par = network.par,step='exp-QC')
...
network.par$net.eset <- net_eset
# QC plots
plot.eset.QC(network.par$net.eset,outdir=network.par$out.dir.QC,intgroup=NULL,do.logtransform=FALSE,prefix='Cluster_')
# save to RData
NetBID2.saveRData(network.par = network.par,step='exp-cluster')

# more cluster functions (will not directly save to file, but avtively layout)
# load from RData
NetBID2.loadRData(network.par = network.par,step='exp-cluster')
#
mat <- exprs(network.par$net.eset)
...
for(i in 1:length(intgroup)){
  print(intgroup[i])
  pred_label <- plot.2D.pca.kmeans(mat=mat,all_k = NULL,obs_label=get_obs_label(phe,intgroup[i]))
  print(table(list(pred_label=pred_label,obs_label=get_obs_label(phe,intgroup[i]))))
}
# pca+keans in 3D
for(i in 1:length(intgroup)){
  print(intgroup[i])
  pred_label <- plot.3D.pca.kmeans(mat=mat,all_k = NULL,obs_label=get_obs_label(phe,intgroup[i]))
  print(table(list(pred_label=pred_label,obs_label=get_obs_label(phe,intgroup[i]))))
}
...
for(i in 1:length(intgroup)){
  print(intgroup[i])
  pred_label <- plot.2D.MICA(outdir=outdir,prjname=prj.name,all_k = 2:6,obs_label=get_obs_label(phe,intgroup[i]))
  print(table(list(pred_label=pred_label,obs_label=get_obs_label(phe,intgroup[i]))))
}

######################################################### Step4: prepare SJARACNE (sjaracne-prep)
# load from RData
NetBID2.loadRData(network.par = network.par,step='exp-QC') ## do not load file from exp-cluster
db.preload(use_level='gene',use_spe='human',update=FALSE)
