...
draw.targetNet(source_label=ms_tab[use_driver,'gene_label'],source_z=ms_tab[use_driver,'Z.G4.Vs.others_DA'],
               edge_score = edge_score,pdf_file=sprintf('%s/targetNet.pdf',analysis.par$out.dir.PLOT),label_cex = 0.2)
...
############# III.1: input driver and target, get shortest path and draw subnetwork (will not be used in pipeline steps)
use_genes <- unique(analysis.par$merge.network$network_dat$target.symbol)
transfer_tab <- get_IDtransfer2symbol2type(from_type = 'external_gene_name',use_genes=use_genes) ## get transfer table !!!
...
sp1 <- get.spByGene(igraph_obj = analysis.par$merge.network$igraph_obj,driver_list = rownames(sig_driver)[1:2],
                    target_list = transfer_tab[which(transfer_tab$external_gene_name %in% c('A1BG')),1],
                    transfer_tab = transfer_tab,
                    plot_pattern = 'random_one')
print(sp1$all_sp) ## print all sp
# can plot saperately
plot.spByGene(sp1$use_sp,driver_list = rownames(sig_driver)[1], target_list = transfer_tab[which(transfer_tab$external_gene_name=='A1BG'),1],
...
sp2 <- get.spByGene(igraph_obj = analysis.par$merge.network$igraph_obj,driver_list = NULL, target_list = transfer_tab[which(transfer_tab$external_gene_name=='A1BG'),1],
                    transfer_tab = transfer_tab,
                    plot_pattern = 'random_one')
print(sp2$all_sp) ## print all sp
# can plot saperately
plot.spByGene(sp2$use_sp,driver_list = NULL, target_list = transfer_tab[which(transfer_tab$external_gene_name=='A1BG'),1],
...
phe_info <- pData(analysis.par$cal.eset)
phe2sample <- vec2list(get_obs_label(phe_info,'subgroup'),sep=NULL)
exp_mat <- exprs(analysis.par$cal.eset) ## expression,the rownames must be the originalID
ac_mat <- exprs(analysis.par$merge.ac.eset) ## ac,the rownames must be the originalID_label
