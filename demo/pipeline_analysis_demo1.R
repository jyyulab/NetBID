...
######### write in paramters
project_main_dir <- sprintf('%s/demo1/',system.file(package = "NetBID2")) ## recommend to use the same directory as in the network generation working space;
#project_main_dir <- 'inst/demo1/'
project_name <-  'driver'
...
#cal_eset <- update.eset.feature(use_eset=cal_eset,use_feature_info=fData(net_eset),from_feature='ID',to_feature='GENE_SYMBOL',merge_method='median') ### !!! need modify
#cal_eset <- update.eset.phenotype(use_eset=cal_eset,use_phenotype_info=pData(net_eset),use_sample_col='geo_accession',use_col='GEO-auto')
